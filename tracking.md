...
Last updated: 2026-09-09 (UI-16/UI-17 and CHAT-03/CHAT-17/CHAT-18 implemented; BUG-05/BUG-07/BUG-08 fixed; focused shell/composer regressions and checkpoint stress checks; protocol remains 45). Target: a C# / WinUI Windows application using **Pi as its only coding-agent runtime**.
...
| TOOL-05 | Windows PowerShell agent tool | Pi | Partial | S | The supported Pi baseline includes the Windows-only `powershell` built-in, and PiStation can pass tool-selection arguments/settings through the runtime while its generic permission/timeline paths handle the tool name. Add dedicated/effective inventory and a real model-invoked PowerShell acceptance check; allowlist UX remains TOOL-07. |
...
Earlier remote docs include automatic updates and non-Windows possibilities. Those are excluded from this workstream by the user's scope, as is cloud account/relay integration. Old reproduced reconnect/payload defects were followed by implementation fixes; do not reopen them solely from the historical review. Physical two-machine acceptance remains DEL-09. The combined protocol 45 requires manually updating both host and client before connecting.
...
| REM-18 | Version compatibility and recovery after manual host update | T3 | Done | H | Combined protocol is now 45; manually update both computers. Preserve useful incompatibility/reconnect guidance. Remote update automation is excluded in SCOPE-08. |
...
| DEL-10 | Regression/compatibility gates and evidence maintenance | Delivery | Partial | V | The 2026-09-09 solution build, shell/composer/locator regressions, and checkpoint contention runs passed, alongside the earlier remote checks (see evidence log). Broader release qualification and BUG-04's native-contention investigation remain; selected tests do not certify every feature. |
