...
Last updated: 2026-09-09 (TOOL-05/TOOL-07 implemented with real-Pi offline acceptance; regression timing failures recorded as BUG-09; native acceptance pending; protocol 46). Target: a C# / WinUI Windows application using **Pi as its only coding-agent runtime**.
...
| PI-20 | Pi provider transport and cache-retention preferences | Pi | Partial | S | Pi exposes stable `transport` and `PI_CACHE_RETENTION` controls, and generic environment configuration can set cache retention. Add discoverable native controls and effective-state verification. Service-tier options currently exist only in the lower-level AI package, not as a stable coding-agent setting; do not copy them into PiStation unless Pi exposes them. |
...
| SES-13 | Tree presentation and entry navigation shortcuts | Pi | Partial | S | Native list selection, indentation and paging exist. Pi's fold/unfold controls, branch-point jumps, filtered tree connector/indent recalculation, copying arbitrary selected entry text, and label timestamp visibility controls remain. Label timestamps are now read and transported; their display toggle is not yet connected. Source: [Pi tree selector](<../Pi Agent/packages/coding-agent/src/modes/interactive/components/tree-selector.ts>). |
...
| HOST-06 | Bitbucket account/repository and PR adapter | T3 | Not started | S | Add supported clone/publish/list/create/review/actions; do not require reopening declined PRs. BUG-07 corrected Settings to label Bitbucket unavailable; the adapter itself remains unimplemented. |
...
| WEB-19 | Browser-link default: system browser versus app preview | T3 | Not started | S | Markdown links currently use the system URI handler. Add a persistent system-browser/app-preview choice independently of tab/profile defaults. |
...
| LOOK-10 | Configurable panel animation duration | T3 | Not started | S | No persisted animation-duration setting or corresponding panel transition was found. Add the preference with reduced-motion behavior and native acceptance. |
...
Earlier remote docs include automatic updates and non-Windows possibilities. Those are excluded from this workstream by the user's scope, as is cloud account/relay integration. Old reproduced reconnect/payload defects were followed by implementation fixes; do not reopen them solely from the historical review. Physical two-machine acceptance remains DEL-09. Protocol 46 includes tool-selection settings/inventory and requires manually updating both host and client before connecting.
...
| DEL-04 | Rendered visual acceptance: scaling, mixed monitors, narrow layouts, rich content | Delivery | Partial | H | Capture/inspect actual populated states; static geometry or black captures do not certify appearance. Include the new resting composer, settings, quit hint, linked PR card, and HEIF previews. |
...
| BUG-07 | HOST-06, DEL-07 | Local/shared | Fixed; native copy acceptance pending | Settings now names only GitHub `gh`, GitLab `glab`, and Azure DevOps `az` as supported adapters, and explicitly labels Bitbucket unavailable. No Bitbucket implementation is claimed. Source/build verified; native Settings inspection remains pending. | [settings copy](src/PiStation.App/Views/ShellPage.xaml), [hosting service](src/PiStation.Host/SourceControl/SourceControlHostingService.cs), [capabilities](src/PiStation.Protocol/Models/HostingCapabilities.cs) |
...
| BUG-09 | DEL-10, REM-05/06, TOOL-07 | Local/shared test environment | Not reproduced in isolated reruns; investigation open | Concurrent broad test/build runs on 2026-09-09 reached 100% CPU and exposed Host startup/subscription/shell-result timeouts, ClientRuntime reconnect/browser-inbox failures, and real-Pi startup timeouts. A reduced-concurrency Host selection still had three timeout failures while another real-Pi run was active. Follow-up selections passed: Host 24, ClientRuntime 10, Pi 0.85.0 10, and Pi 0.84.4 5. CPU contention is an observation, not a proven root cause. No production fix or clean current full-suite gate is claimed. | [host resources](tests/PiStation.Host.Tests/PiResourcesIntegrationTests.cs), [host stop](tests/PiStation.Host.Tests/EmbeddedHostIntegrationTests.cs), [shell recovery](tests/PiStation.Host.Tests/PiShellTests.cs), [remote recovery](tests/PiStation.ClientRuntime.Tests/RemoteAccessIntegrationTests.cs), [browser transport](tests/PiStation.ClientRuntime.Tests/BrowserAutomationTransportTests.cs) |
...
- **2026-09-09, TOOL-05/TOOL-07 implementation:** native host-scoped controls, normalized persistence/launch flags, planning aliases, child-policy intersection, truthful registered/active inventory, and Windows PowerShell preset support are connected. See [policy contract](src/PiStation.Protocol/Models/PiTools.cs), [desktop logic](src/PiStation.App/ViewModels/PiToolsViewModel.cs), [runtime launch](src/PiStation.Host/Threads/IPiProcessFactory.cs), [real-Pi tests](tests/PiStation.PiRpc.Tests/RealPiToolSelectionTests.cs), and [usage/acceptance](Docs/PI-TOOL-CONFIGURATION-2026-09-09.md). The wire contract is now **46**; older protocol statements below are historical. Dedicated selection requires Pi **0.85.0+**; Pi defaults without dedicated exclusions preserve the supported **0.84.4** workflow. No global Pi installation or shared Pi settings were changed.
