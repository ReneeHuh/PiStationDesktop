...
{
    [Fact]
    public async Task ContextOnlyDraftSurvivesRecoveryIncludingTypedSourceIdentifiers()
    {
        if (!OperatingSystem.IsWindows()) return;
        using var directory = new ClientTestDirectory();
        var root = directory.CreateDirectory("context-recovery");
        var baseline = new PiStation.Protocol.Models.ComposerContext("old", "file", "Old file", "original");
        var context = new PiStation.Protocol.Models.ComposerContext("new", "citation", "Reply", "quoted text",
            PiStation.Protocol.Identifiers.ThreadId.New(), "message", "a.cs", 2, 3, "sha", "comment");
        var store = new EditingRecoveryStore(root);
        store.SaveDraft(RecoveredDraft.Capture("thread", "draft", "", "", [baseline], [context]));
        await store.FlushAsync();
        var recovered = new EditingRecoveryStore(root).LoadDraft("thread");
        Assert.NotNull(recovered);
        Assert.Equal(baseline, Assert.Single(recovered.BaseContext!));
        Assert.Equal(context, Assert.Single(recovered.Context!));
