...
| PI-09 | Pi-dependent | Native plan/execute workflow, proposed plan card/export, progress/tasks, and genuinely enforced permission modes. Pi core does not supply T3's supervised/auto/full-access modes; its plan-mode example is an extension. Current runtime mode capability list is empty. | Show a plan, approve execution, track steps, and prove the chosen policy controls tools. Do not represent a prompt instruction as enforcement. |
...
| DESK-04 | Partial | Rich **sent** attachments and media: durable file identities, clickable previews/lightbox, save/copy path/URL, local workspace images and video behavior; HEIC/HEIF conversion. Draft image previews and file-panel media viewers already exist. Sent messages currently reduce attachments to filename text. | Send an image/video/file, restart, and reopen/save it from the original message. Convert an HEIC photo before provider dispatch. |
| DESK-05 | Partial | Clickable citations in **sent** user messages with preserved source metadata and comments. Draft citations can navigate, but user timeline rows currently carry/render text without a structured sent-citation model. | Send a citation, reopen the conversation, and navigate back to its original response/file range; a missing source preserves the quote. |
...
| ID | Status | Work to finish | Completion check |
| --- | --- | --- | --- |
| GIT-01 | Partial | Full PR workspace: cross-project/provider filtering, PR detail/body, commit/activity/check information, hosted diffs, checkout/review-thread creation and branch linkage. The current entry point mainly lists/creates/links PRs and opens the host website. Local Changes diffs are already implemented. | Review a PR that was not created by the selected local thread without leaving PiStation. |
| GIT-02 | Partial | Read conversations; edit PR title/body and own comments; inline review drafts/submission, replies, resolve/unresolve and reactions where the host supports them. Existing generic comment/approve/request-changes actions are only part of this. | Complete an inline review with replies and resolved threads; reopening displays the server's authoritative state. |
