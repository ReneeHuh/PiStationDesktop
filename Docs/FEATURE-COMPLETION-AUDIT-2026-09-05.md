...
**Scope update, September 6:** PiStation is a Pi-only Windows tool. Additional coding-agent runtimes and multi-runtime account instances are outside product scope. The full T3 comparison below is historical context; remote environments and non-Windows clients are also excluded from the current implementation work.

PiStation has a substantial local coding workspace. It does **not** yet have full T3 feature parity. Following the first Pi integration milestone, the immediate gaps include advanced Pi runtime setup, richer conversation artifacts and full pull-request review. Custom themes are another remaining product area.

**Implementation follow-up:** the [first Pi integration milestone](PI-INTEGRATION-MILESTONE-2026-09-05.md) implements skill expansion/current metadata, configurable extension loading, supported text-based extension UI and idle restart. PI-01 through PI-03 below include that progress. The [resource and provider setup follow-up](PI-RESOURCES-AND-SETUP-2026-09-06.md) adds PI-04/05 implementation and Pi 0.85/authenticated acceptance. The [session management follow-up](PI-SESSION-MANAGEMENT-2026-09-06.md) adds session browsing/import, independent copies and response forks, tree/statistics inspection and JSONL/HTML export. The [plan workflow follow-up](PI-PLAN-WORKFLOW-2026-09-06.md) adds native planning, saved revision approval, enforced Pi tool restrictions, progress, Markdown export and recovery. The [agent workflow follow-up](PI-AGENT-WORKFLOWS-2026-09-06.md) adds bundled SDK delegation, preset management, single/parallel/chain workflows, child transcripts, targeted interruption and continuation. The other feature families remain backlog; this is not a full-parity completion claim.
...
| Agents panel | Bundled single/parallel/chain workflows, reusable presets, hierarchy/progress/metrics, child transcripts, targeted interruption and saved-context continuation; external extensions retain parent-turn interruption | [Agent projection](../src/PiStation.Host/Threads/PiAgentActivityProjector.cs), [Agents UI](../src/PiStation.App/ViewModels/WorkbenchAgentsViewModel.cs) |
...
The bundled subagent integration now produces real Agents activity; more precise per-resource failure attribution remains. Native resource/trust inventory, Pi 0.85 execution and authenticated-provider smoke coverage are now recorded in the September 6 follow-up. An Agents panel that recognizes subagent output alone does not establish subagent workflow parity.
...
| PI-10 | Implemented core; acceptance limits | Bundled Pi SDK sessions, native presets, single/parallel/chain requests, child transcripts, targeted Stop and explicit continuation from saved context. External extensions without PiStation child handles retain parent-turn Stop. | Actual Pi 0.84.4/0.85.0 tests cover all modes, failure, cancellation, continuation, setup and planning guards. Native/host acceptance is recorded in the agent follow-up. Broader authenticated providers, aggregate child billing and process/remote isolation remain. |
...
| ID | Status | Work to finish | Completion check |
| --- | --- | --- | --- |
| DESK-01 | Implemented core; native acceptance pending | Durable completion/read counters, separate Unread labels, automatic read marking for displayed completions in the active window, and single/bulk Mark unread. Old history stays unmarked; stale reads cannot clear newer work. See [read state](THREAD-READ-STATE-2026-09-07.md). | An unseen completion is marked, visiting clears it, manual unread works, and restart preserves it. |
| DESK-02 | Implemented core; acceptance/parity limits | Host-owned startup/minute sweeps, persisted inactivity/merged/closed-PR settings, activity/approval/background-work safeguards, durable un-settle and plan protection, revision-guarded writes and unpinning. See [automatic settlement](AUTO-SETTLEMENT-2026-09-07.md). | Native/authenticated acceptance pending. Conservative legacy/snooze handling, bounded PR lists and no immediate merge-event sweep; embedded host must stay open. |
| DESK-03 | Implemented core; native acceptance pending | **New task** creates an independent Pi thread/worktree using selected-thread project/model/branch/workspace defaults, preserving sent context and safely clearing the accepted draft. Durable submission receipts prevent duplicate retry dispatch. See [background tasks](BACKGROUND-TASKS-2026-09-07.md). | Integration coverage for three successive local/worktree tasks and restart replay; native composer click/keyboard/layout acceptance remains. |
...
| --- | --- | --- | --- |
| RUNTIME-01 | Out of scope | Additional coding-agent runtimes alongside Pi. | PiStation uses Pi as its sole coding-agent runtime. Pi's supported model providers remain available through Pi. |
| RUNTIME-02 | Out of scope | Multi-runtime account instances and switching between agent harnesses. | Pi account/provider setup remains part of Pi's existing integration. |
...
| --- | --- | --- |
| QA-01 | Partial acceptance | Actual Pi 0.84.4/0.85.0 offline tests cover skills/templates, tool execution, extension commands/UI, discovery, resource/package toggles, trust, custom-model persistence and resume. Authenticated skill invocation and resumed turns passed on both versions. Remaining: broader providers, first-login onboarding, model/thinking changes, attachments, queue delivery, compaction, authenticated subagents and browser automation. |
