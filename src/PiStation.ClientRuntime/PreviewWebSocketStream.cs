...
internal sealed class PreviewWebSocketStream(ClientWebSocket socket) : Stream
{
    ...
    {
        // HttpClient probes pooled connections with zero-byte reads. A WebSocket receive
        // cannot fill an empty buffer, so looping on it would stall the next HTTP request.
