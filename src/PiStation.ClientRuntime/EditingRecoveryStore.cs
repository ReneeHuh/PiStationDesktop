...
public sealed record RecoveredDraft(string ThreadId, string DraftId, string BaseText, string Text,
    JsonElement? BaseContextData = null, JsonElement? ContextData = null)
{
    [JsonIgnore] public IReadOnlyList<ComposerContext>? BaseContext => ReadContext(BaseContextData);
    [JsonIgnore] public IReadOnlyList<ComposerContext>? Context => ReadContext(ContextData);

    public static RecoveredDraft Capture(string threadId, string draftId, string baseText, string text,
        IReadOnlyList<ComposerContext>? baseContext, IReadOnlyList<ComposerContext> context) => new(threadId, draftId, baseText, text,
            JsonSerializer.SerializeToElement(baseContext ?? [], typeof(IReadOnlyList<ComposerContext>), ProtocolJsonContext.Default),
            JsonSerializer.SerializeToElement(context, typeof(IReadOnlyList<ComposerContext>), ProtocolJsonContext.Default));

    private static IReadOnlyList<ComposerContext>? ReadContext(JsonElement? value) => value is { } element
        ? (IReadOnlyList<ComposerContext>?)JsonSerializer.Deserialize(element, typeof(IReadOnlyList<ComposerContext>), ProtocolJsonContext.Default)
        : null;
}
