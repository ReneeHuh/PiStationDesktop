...
export default function registerSessions(pi: any) {
  pi.registerCommand("pistation-desktop-sessions", {
    description: "PiStation session branch navigation",
    handler: async (payload: string, ctx: any) => {
      ...
      try {
        ...
        if (!/^[a-f0-9]{32}$/.test(request.id ?? "") || !/^[a-f0-9]{32}$/.test(request.operationId ?? "") ||
            ...
            request.entryId.length > 256 || typeof request.summarize !== "boolean" ||
        ...
        const previous = entries.find((entry: any) => entry.type === "custom" && entry.customType === markerType &&
