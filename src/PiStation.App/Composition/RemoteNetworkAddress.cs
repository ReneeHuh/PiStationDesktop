    ...
    public static IReadOnlyList<RemoteNetworkAddress> Discover() => NetworkInterface.GetAllNetworkInterfaces()
        ...
        .SelectMany(adapter => adapter.GetIPProperties().UnicastAddresses
            ...
            .Select(item => new RemoteNetworkAddress(item.Address.ToString(),
                $"{item.Address} — {adapter.Name} ({(PiStation.ClientRuntime.TailscaleDiscovery.IsTailscaleAddress(item.Address.ToString()) ? "Tailscale / VPN" : adapter.NetworkInterfaceType.ToString())}" +
