...
export function renderGhosttySnapshot(options: {
  ...
  readonly searchMatches?: readonly GhosttyCellRange[];
  readonly activeSearchMatch?: GhosttyCellRange | null;
  ...
  const searchMatches = options.searchMatches ?? [];
  const activeSearchMatch = options.activeSearchMatch ?? null;
  ...
  for (const rowIndex of rowsToDraw) {
    ...
    for (const match of searchMatches) {
      if (rowIndex < match.start.y || rowIndex > match.end.y) continue;
      const start = rowIndex === match.start.y ? match.start.x : 0;
      const end = rowIndex === match.end.y ? match.end.x : snapshot.cols - 1;
      context.fillStyle = match === activeSearchMatch
        ? "rgba(255, 174, 66, 0.72)"
        : "rgba(246, 211, 101, 0.38)";
      context.fillRect(
        padding + start * metrics.width,
        top,
        Math.max(1, end - start + 1) * metrics.width,
        metrics.height,
      );
    }

    let selectionStart = 0;
    while (selectionStart < row.cells.length) {
      if (!row.cells[selectionStart]?.selected) {
        selectionStart += 1;
        continue;
      }
      let selectionEnd = selectionStart + 1;
      while (selectionEnd < row.cells.length && row.cells[selectionEnd]?.selected) {
        selectionEnd += 1;
      }
      context.fillStyle = selectionBackground;
      context.fillRect(
        padding + selectionStart * metrics.width,
        top,
        (selectionEnd - selectionStart) * metrics.width,
        metrics.height,
      );
