...
import symbolsFontUrl from "./fonts/SymbolsNerdFontMono-Regular.woff2";
import { isMonospaceFamily } from "../font";
import {
  buildGhosttySearchDocument,
  findGhosttySearchMatches,
  type GhosttySearchMatch,
  type GhosttySearchOptions,
} from "./search";
...
export interface GhosttyTerminalFontState {
  readonly family: string;
  readonly size: number;
...
export interface GhosttyTerminalSurfaceOptions {
  ...
  readonly onSearchStateChange?: (state: GhosttyTerminalSearchState) => void;
...
export class GhosttyTerminalSurface {
  ...
  private inputTop = -1;
  private searchQuery = "";
  private searchOptions: GhosttySearchOptions = { caseSensitive: false, wholeWord: false };
  private searchMatches: readonly GhosttySearchMatch[] = [];
  private activeSearchIndex = -1;
  private searchTimer: number | null = null;
  private searchShouldResetActive = false;
  ...
  async setFont(font: GhosttyTerminalFont): Promise<GhosttyTerminalFontState | null> {
    ...
    if (this.disposed || epoch !== this.fontEpoch) return null;
    this.pendingFontEpoch = null;
    this.fontFamily = fontFamily;
    this.requestedFontFamily = font.family;
    this.fontSize = fontSize;
    this.applyFontMetrics();
    const family = font.family?.trim() || "Default monospace";
    return {
      family,
      size: fontSize,
      usedFallback: family !== "Default monospace" && fontFamily === DEFAULT_TERMINAL_FONT_FAMILY,
    };
  }
  ...
  setSearch(query: string, options: GhosttySearchOptions): void {
    if (this.disposed) return;
    const changed =
      query !== this.searchQuery ||
      options.caseSensitive !== this.searchOptions.caseSensitive ||
      options.wholeWord !== this.searchOptions.wholeWord;
    ...
    this.scheduleSearch(changed);
  }

  navigateSearch(direction: 1 | -1): void {
    if (this.disposed || this.searchQuery.length === 0) return;
    this.refreshSearchIfPending();
  ...
  clearSearch(): void {
    ...
    this.searchShouldResetActive = false;
    this.searchQuery = "";
    this.searchMatches = [];
  ...
  getAccessibleText(): string {
    const snapshot = this.snapshot ?? this.core.snapshot();
    return snapshot.rowData.map((row) => row.text.replace(/\s+$/u, "")).join("\n").trimEnd();
  }
  ...
  private scheduleSearch(resetActive: boolean): void {
    if (this.searchQuery.length === 0 || this.disposed) return;
    this.searchShouldResetActive ||= resetActive;
    if (this.searchTimer !== null) window.clearTimeout(this.searchTimer);
  ...
  private refreshSearchIfPending(): void {
  ...
  private refreshSearch(): void {
    if (this.disposed || this.searchQuery.length === 0) return;
    const initialState = this.core.scrollbarState();
    ...
    const rows = this.captureSearchRows(initialState);
    const document = buildGhosttySearchDocument(rows);
    const previous = this.searchMatches[this.activeSearchIndex];
    this.searchMatches = findGhosttySearchMatches(document, this.searchQuery, this.searchOptions);

    if (this.searchMatches.length === 0) {
      this.activeSearchIndex = -1;
    } else if (!this.searchShouldResetActive && previous) {
      const previousIndex = this.searchMatches.findIndex(
        (match) => match.start.x === previous.start.x && match.start.y === previous.start.y,
      );
      this.activeSearchIndex = previousIndex >= 0 ? previousIndex : 0;
    } else {
      const firstVisibleOrLater = this.searchMatches.findIndex(
        (match) => match.start.y >= initialState.offset,
      );
      this.activeSearchIndex = firstVisibleOrLater >= 0 ? firstVisibleOrLater : 0;
    }
    this.searchShouldResetActive = false;
    this.revealActiveSearchMatch();
    this.publishSearchState();
  ...
  private captureSearchRows(initialState: GhosttyScrollbar): Array<GhosttySnapshot["rowData"][number] | undefined> {
    const total = Math.max(initialState.total, initialState.len);
    const pageLength = Math.max(1, initialState.len);
    const maxOffset = Math.max(0, total - pageLength);
    const rows: Array<GhosttySnapshot["rowData"][number] | undefined> = new Array(total);
    let currentOffset = initialState.offset;
    const offsets: number[] = [];
    for (let offset = 0; offset < maxOffset; offset += pageLength) offsets.push(offset);
    offsets.push(maxOffset);

    for (const offset of offsets) {
      if (offset !== currentOffset) this.core.scroll(offset - currentOffset);
      currentOffset = offset;
      const snapshot = this.core.snapshot();
      for (let row = 0; row < snapshot.rowData.length && offset + row < total; row += 1) {
        rows[offset + row] = snapshot.rowData[row];
      }
    }

    if (initialState.offset !== currentOffset) this.core.scroll(initialState.offset - currentOffset);
    this.snapshot = this.core.snapshot();
    this.scrollbarState = this.core.scrollbarState();
    this.forceFullRender = true;
    this.scrollbarDirty = true;
    return rows;
  }
  ...
  private revealActiveSearchMatch(): void {
    ...
    const maximum = Math.max(0, state.total - state.len);
    const desired = Math.max(0, Math.min(match.start.y - Math.floor(state.len / 2), maximum));
    if (desired !== state.offset) this.core.scroll(desired - state.offset);
    this.scrollbarState = this.core.scrollbarState();
    this.forceFullRender = true;
    this.scrollbarDirty = true;
    this.requestRender();
  ...
  private publishSearchState(): void {
    this.options.onSearchStateChange?.({
      query: this.searchQuery,
  ...
  private visibleSearchMatches(): {
    readonly matches: readonly GhosttyCellRange[];
    readonly active: GhosttyCellRange | null;
  } {
    const state = this.scrollbarState;
    if (state === null || this.searchMatches.length === 0) return { matches: [], active: null };
    const ranges: GhosttyCellRange[] = [];
    let active: GhosttyCellRange | null = null;
    const viewportEnd = state.offset + state.len - 1;
    for (let index = 0; index < this.searchMatches.length; index += 1) {
      const match = this.searchMatches[index];
      if (!match || match.end.y < state.offset || match.start.y > viewportEnd) continue;
      const range: GhosttyCellRange = {
        start: {
          x: match.start.y < state.offset ? 0 : match.start.x,
          y: Math.max(0, match.start.y - state.offset),
        },
        end: {
          x: match.end.y > viewportEnd ? this.cols - 1 : match.end.x,
          y: Math.min(this.rows - 1, match.end.y - state.offset),
        },
      };
      ranges.push(range);
      if (index === this.activeSearchIndex) active = range;
    }
    return { matches: ranges, active };
  }
  ...
  private renderFrame(): void {
    ...
    this.refreshHoveredLink();
    const visibleSearch = this.visibleSearchMatches();
    renderGhosttySnapshot({
      ...
      searchMatches: visibleSearch.matches,
      activeSearchMatch: visibleSearch.active,
