...
internal static class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        try
        {
            ...
            var pi = await new PiLocator().LocateAsync(new PiLocatorOptions { ExplicitPiPath = piPath }, startup.Token).ConfigureAwait(false);
            SshEnvironmentHost? host = null;
            try
            {
                host = await SshEnvironmentHost.StartAsync(new HostOptions
                {
                    ApplicationDataRoot = root, EnvironmentName = Environment.MachineName, PiInstallation = pi,
                }, diagnosticLog: message => Console.Error.WriteLine(message), sharingAddress: sharingAddress, sharingPort: sharingPort, cancellationToken: startup.Token).ConfigureAwait(false);
            }
