...
public sealed partial class HostDatabase
{
    ...
    private static async Task InitializeCatalogAsync(SqliteConnection connection, CancellationToken cancellationToken)
    {
        ...
        foreach (var (table, key, kind) in new[] { ("Projects", "ProjectId", "project"), ("Threads", "ThreadId", "thread"),
                     ("ThreadInboxMetadata", "ThreadId", "thread"), ("ThreadDrafts", "ThreadId", "thread") })
        ...
        foreach (var operation in new[] { "INSERT", "UPDATE", "DELETE" })
        {
            var row = operation == "DELETE" ? "OLD" : "NEW";
            schema.CommandText = $"""
                CREATE TRIGGER IF NOT EXISTS Catalog_DraftAttachments_{operation} AFTER {operation} ON DraftAttachments
                BEGIN
                    INSERT INTO CatalogChanges(Kind, EntityId)
                        SELECT 'thread', ThreadId FROM ThreadDrafts WHERE DraftId = {row}.DraftId;
                    DELETE FROM CatalogChanges WHERE Sequence <= (SELECT MAX(Sequence) FROM CatalogChanges) - {CatalogRetention};
                END;
                """;
            await schema.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false);
        }
    ...
    private async Task<ProjectDescriptor> ReadCatalogProjectAsync(SqliteDataReader reader, CancellationToken cancellationToken)
    {
        var project = ReadProject(reader);
        return project with { RepositoryKey = await ProjectRepositoryIdentity.ReadAsync(project.CanonicalPath, cancellationToken).ConfigureAwait(false) };
    }
        ...
        if (reset)
        {
            ...
            command.CommandText = """
                SELECT ProjectId, CanonicalPath, DisplayName, CreatedUtc, DefaultWorkspaceMode, ScriptsJson, AreRepositoryScriptsTrusted,
                               Icon, DefaultModelProvider, DefaultModelId, DefaultThinkingLevel, DefaultRuntimeModeId, AutoPullDefaultBranch
            ...
            await using (var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    page.Add(await ReadCatalogProjectAsync(reader, cancellationToken).ConfigureAwait(false));
            ...
            await using (var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
                {
                    page.Add(await EnrichThreadDescriptorAsync(connection, transaction, ReadThread(reader), cancellationToken).ConfigureAwait(false));
            ...
            foreach (var (kind, id) in changes)
            {
                ...
                if (kind == "project")
                {
                    ...
                    yield return await reader.ReadAsync(cancellationToken).ConfigureAwait(false)
                        ? Page(projects: [await ReadCatalogProjectAsync(reader, cancellationToken).ConfigureAwait(false)]) : Page(removedProjects: [new ProjectId(id)]);
                }
                else
                {
                    command.CommandText = """
                    ...
                    await using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
                    yield return await reader.ReadAsync(cancellationToken).ConfigureAwait(false)
                        ? Page(threads: [await EnrichThreadDescriptorAsync(connection, transaction, ReadThread(reader), cancellationToken).ConfigureAwait(false)]) : Page(removedThreads: [new ThreadId(id)]);
                }
            }
        }
